...
// config
settings.set('trayUpdateInterval', 1000 * 10);
settings.set('fetchInterval', 1000 * 60);
settings.set('trayIgnoreBefore', 8);
settings.set('stopID', '878');
settings.set('routeID', '11');
...
function startTrayTextUpdate() {
	stopTrayTextUpdate();
	setLastUpdated();
	trayTextUpdateTimeout = setTimeout(startTrayTextUpdate, settings.get('trayUpdateInterval'));
}
...
function setTrayText() {
	...
	const results = rti.data.results.filter(function (result) {
		return result.duetime >= settings.get('trayIgnoreBefore');
	});
	let dueTime;
	if (!results.length) {
...
function fetch(cb) {
	api.getRTI(settings.get('stopID'), settings.get('routeID'), function (err, data) {
		if (err) {
			log.error('ERROR:', err);
			return cb && cb(err);
		} else {
...
function startFetchUpdate() {
	stopFetchUpdate();
	fetch();
	fetchUpdateTimeout = setTimeout(startFetchUpdate, settings.get('fetchInterval'));
}
